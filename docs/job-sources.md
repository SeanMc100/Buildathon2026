...
`npm run ingest:jobs` builds `data/jobs.json` from four sources and writes 120
items: 102 common Detroit occupations and 18 standing apprenticeships/internships.
...
|---|---|---|---|
| **O*NET 31.0** | Bulk CSV tables from `onetcenter.org/dl_files/database/db_31_0_csv/` | No | Every match tag: Holland code, job zone, education, work context, activities. 102 items. |
| **BLS OEWS May 2025** | `oesm25ma.zip` → `MSA_M2025_dl.xlsx`, filtered to MSA 19820 | No | Detroit wage percentiles, local employment, location quotient. 119 of 120 items. |
| **Projections Central** | `public.projectionscentral.org/Projections/LongTermRestJson/26`, paged | No | Michigan 2024–2034 growth and annual openings. 109 items. |
...
- **demands** — measured for `physical_work`, `managing_people`,
  ...
  - `night_shifts` — a curated list of SOC families that run around the clock.
    ...
    schedule field cannot see it. This is the most over-inclusive tag (46 of
    102 occupations) and the first one to argue with.
...
`select.ts` keeps only the occupations named in `common-roles.ts`: the roles a
person would recognise (Software Engineer, Financial Manager, Electrician,
Cashier), each with the title they would type rather than the federal one. BLS
lists ~700 detailed occupations for the metro; a cut by pay gives doctors and
executives, and a cut by headcount still lets in Allergists and Regulatory
Affairs Managers, so the cut is editorial. Local demand (employment 55%,
statewide annual openings 30%, location quotient 15%) orders the list; it does
not decide who is on it. A minimum of 300 local jobs keeps a listed role from
being a Detroit rarity, and any role on the list that a source failed to produce
is named in the run's dropped report.

To add or remove a role, edit `common-roles.ts` and re-run `npm run ingest:jobs`.

Employer programmes (internships, apprenticeships, "entry roles at X") are not
occupations. They come from `entry-routes.ts` and the app lists them apart from
the jobs above.
